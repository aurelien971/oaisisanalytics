...
// ── Drill-down: filter tap → user → sessions → screens ──────────────────────

/** The live tap feed: what was tapped, by whom, when. */
export function filterFeed(events, n = 80) {
  return events
    .filter((e) => e.type === "filter_tap" || e.type === "filter_apply")
    .slice(0, n)
    .map((e) => ({
      uid: e.uid,
      at: e.tsMs,
      day: e.day,
      name: e.filterName || e.filterId || "—",
      id: e.filterId || null,
      kind: e.kind || "—",
      applied: e.type === "filter_apply",
    }));
}
...
/** One user's sessions. No orderBy — avoids needing a composite index. */
export async function getUserSessions(uid, limit = 200) {
  const snap = await db().collection("sessions").where("uid", "==", uid).limit(limit).get();
  return snap.docs
    .map((d) => {
      const x = d.data();
      return {
...
/** Everything one user did, newest first — the raw timeline. */
export function userEvents(events, uid, n = 200) {
  return events.filter((e) => e.uid === uid).slice(0, n);
}

// ── Generations ─────────────────────────────────────────────────────────────
// generation_completed now carries beforePath/afterPath once the app version
// that archives them is out. Older events have neither, so `hasImages` says
// whether a row can actually be looked at.

export function generationsFrom(events, n = 200) {
  return events
    .filter((e) => e.type === "generation_completed" || e.type === "generation_failed")
    .slice(0, n)
    .map((e) => ({
      uid: e.uid,
      at: e.tsMs,
      day: e.day,
      failed: e.type === "generation_failed",
      ...
      costUSD: e.costUSD ?? 0,
      error: e.error || null,
      beforePath: e.beforePath || null,
      afterPath: e.afterPath || null,
      hasImages: !!(e.beforePath || e.afterPath),
    }));
}

/** Generations that happened inside a session's window. */
export function generationsInSession(session, generations) {
  const from = session.startMs ?? 0;
  const to = from + (session.duration || 0) * 1000;
  return generations
    .filter((g) => g.uid === session.uid && g.at >= from - 2000 && g.at <= to + 2000)
    .sort((a, b) => a.at - b.at);
}
