// Reads and derived metrics for the three products beyond Opaque.
//
// Opaque keeps its own lib/data.js — it's older, bigger and has its own shape.
// Everything here follows the same pattern: fetch the raw collections, then
// derive in plain functions so a page can stay declarative.
import { db } from "./firebase";

/* ── helpers ─────────────────────────────────────────────────────────── */
import { toUSD } from "./money";

const rows = (snap) => snap.docs.map((d) => ({ id: d.id, ...d.data() }));
const ms = (t) => (t?.toMillis ? t.toMillis() : t ? new Date(t).getTime() : 0);
const dayKey = (t) => (ms(t) ? new Date(ms(t)).toISOString().slice(0, 10) : null);

/** Counts per day for the last `days`, oldest first — chart-ready. */
export function daily(items, field, days = 30) {
  const out = new Map();
  for (let i = days - 1; i >= 0; i--) {
    const d = new Date();
    d.setDate(d.getDate() - i);
    out.set(d.toISOString().slice(0, 10), 0);
  }
  for (const it of items) {
    const k = dayKey(it[field]);
    if (k && out.has(k)) out.set(k, out.get(k) + 1);
  }
  return [...out].map(([name, value]) => ({ name: name.slice(5), value }));
}

export const countBy = (items, key, limit = 12) => {
  const m = new Map();
  for (const it of items) {
    const v = typeof key === "function" ? key(it) : it[key];
    if (v === undefined || v === null || v === "") continue;
    m.set(v, (m.get(v) ?? 0) + 1);
  }
  return [...m.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
    .map(([name, value]) => ({ name: String(name), value }));
};

export const fmtN = (v) =>
  v >= 1e6 ? `${(v / 1e6).toFixed(1)}M` : v >= 1e3 ? `${(v / 1e3).toFixed(1)}K` : String(Math.round(v ?? 0));

export const ago = (t) => {
  const n = ms(t);
  if (!n) return "—";
  const s = Math.floor((Date.now() - n) / 1000);
  if (s < 60) return `${s}s ago`;
  if (s < 3600) return `${Math.floor(s / 60)}m ago`;
  if (s < 86400) return `${Math.floor(s / 3600)}h ago`;
  return `${Math.floor(s / 86400)}d ago`;
};

export const when = (t) => (ms(t) ? new Date(ms(t)).toLocaleString() : "—");

/* ── OAISIS Labs — the TikTok scheduler ──────────────────────────────── */
export async function oaisisLabs() {
  const d = db("oaisislabs");
  const [posts, users] = await Promise.all([
    d.collection("posts").get().then(rows),
    d.collection("users").get().then(rows),
  ]);

  const by = (s) => posts.filter((p) => p.status === s);
  const delivered = posts.filter((p) => p.deliveredAt);
  const photo = posts.filter((p) => p.mediaType === "PHOTO");

  // How long TikTok took between accepting a post and confirming delivery.
  const lags = delivered
    .map((p) => (ms(p.deliveredAt) - ms(p.postedAt)) / 1000)
    .filter((n) => n > 0 && n < 86400);
  const medianLag = lags.length ? lags.sort((a, b) => a - b)[Math.floor(lags.length / 2)] : null;

  return {
    posts,
    users,
    acquisition: acquisitionOf(users),
    new7d: newSince(users, 7),
    kpis: {
      users: users.length,
      posts: posts.length,
      posted: by("posted").length,
      scheduled: by("scheduled").length,
      failed: by("failed").length,
      drafts: by("draft").length,
      delivered: delivered.length,
      slideshows: photo.length,
      medianLag,
      direct: posts.filter((p) => p.mode === "direct").length,
      inbox: posts.filter((p) => p.mode === "inbox").length,
    },
    byStatus: countBy(posts, "status"),
    byPrivacy: countBy(posts, "privacy"),
    perDay: daily(posts, "createdAt"),
    failures: by("failed").map((p) => ({ id: p.id, name: p.name, error: p.error, at: p.postedAt })),
  };
}

/* ── OAISIS Transcriber ──────────────────────────────────────────────── */
export async function oaisisTranscriber() {
  const d = db("oaisis");
  // 3.5k+ transcriptions: pull a recent window rather than the whole history.
  const [users, transcriptions, optimizations] = await Promise.all([
    d.collection("users").get().then(rows),
    d.collection("transcriptions").orderBy("timestamp", "desc").limit(2000).get().then(rows),
    d.collection("optimizations").orderBy("timestamp", "desc").limit(500).get().then(rows),
  ]);

  const seconds = transcriptions.reduce((a, t) => a + (t.audioDurationSeconds ?? 0), 0);
  const words = users.reduce((a, u) => a + (u.totalWordsTranscribed ?? 0), 0);
  const pro = users.filter((u) => u.isProMember);
  const active = users.filter((u) => Date.now() - ms(u.lastLogin) < 7 * 864e5);

  const perUser = [...users]
    .map((u) => ({
      id: u.id,
      name: u.name || "—",
      email: u.email || "—",
      pro: !!u.isProMember,
      words: u.totalWordsTranscribed ?? 0,
      opts: u.totalOptimizations ?? 0,
      wordsCycle: u.wordsUsedThisCycle ?? 0,
      created: u.dateCreated,
      last: u.lastLogin,
      transcriptions: transcriptions.filter((t) => t.uid === u.id).length,
    }))
    .sort((a, b) => b.words - a.words);

  return {
    users,
    transcriptions,
    optimizations,
    acquisition: acquisitionOf(users),
    new7d: newSince(users, 7),
    kpis: {
      users: users.length,
      pro: pro.length,
      proRate: users.length ? pro.length / users.length : 0,
      active7d: active.length,
      transcriptions: transcriptions.length,
      optimizations: optimizations.length,
      hours: seconds / 3600,
      words,
      avgClipS: transcriptions.length ? seconds / transcriptions.length : 0,
    },
    perDay: daily(transcriptions, "timestamp"),
    optsPerDay: daily(optimizations, "timestamp"),
    perUser,
    recent: transcriptions.slice(0, 40),
    recentOpts: optimizations.slice(0, 30),
  };
}

/* ── FAIKE ───────────────────────────────────────────────────────────── */
export async function faike() {
  const d = db("faike");
  const [users, sessions, events] = await Promise.all([
    d.collection("users").get().then(rows),
    d.collection("sessions").orderBy("started_at", "desc").limit(500).get().then(rows),
    d.collection("events").orderBy("ts", "desc").limit(1000).get().then(rows),
  ]);

  const scans = (u) => (u.scans_image ?? 0) + (u.scans_text ?? 0) + (u.scans_fact_check ?? 0);
  const totalScans = users.reduce((a, u) => a + scans(u), 0);
  const converted = users.filter((u) => u.paywall_converted);
  const sawPaywall = users.filter((u) => (u.paywall_views ?? 0) > 0);
  const durations = sessions.map((s) => s.duration_s ?? 0).filter(Boolean).sort((a, b) => a - b);

  // ── Money ──────────────────────────────────────────────────────────────
  // The charged price and its currency live on the purchase event, not the
  // user doc. Normalise to USD before summing: a GBP 19.99 is not $19.99.
  const LIST = { monthly: 3.99, yearly: 19.99 };
  const purchases = events
    .filter((e) => e.name === "purchase_success")
    .map((e) => {
      const q = e.params || {};
      const native = typeof q.price === "number" ? q.price : LIST[q.plan] || 0;
      return { uid: e.uid, at: ms(e.ts), plan: q.plan || "unknown", native,
               currency: q.currency || "USD", amountUSD: toUSD(native, q.currency || "USD"),
               estimated: false };
    });
  const priced = new Set(purchases.map((x) => x.uid));
  for (const u of users) {
    if (!u.paywall_converted || priced.has(u.id) || !u.converted_at) continue;
    const plan = u.conversion_plan || u.sub_plan || "monthly";
    purchases.push({ uid: u.id, at: ms(u.converted_at), plan, native: LIST[plan] || LIST.monthly,
                     currency: "USD", amountUSD: LIST[plan] || LIST.monthly, estimated: true });
  }
  purchases.sort((a, b) => (b.at || 0) - (a.at || 0));

  const TOKEN = { "gpt-4o": { in: 2.5, out: 10 }, "gpt-4o-mini": { in: 0.15, out: 0.6 } };
  const FALLBACK = { fact_check: 0.04, image: 0.012, text: 0.005, follow_up: 0.002 };
  const withTokens = new Set(users.filter((u) => (u.api_calls || 0) > 0).map((u) => u.id));
  let cost = 0;
  for (const e of events) {
    if (e.name !== "api_usage") continue;
    const q = e.params || {};
    if (q.endpoint === "whisper") { cost += 0.006; continue; }
    const pr = TOKEN[q.model] || TOKEN["gpt-4o"];
    cost += ((q.tokens_in || 0) / 1e6) * pr.in + ((q.tokens_out || 0) / 1e6) * pr.out;
    if (q.web_search) cost += 0.025;
  }
  for (const u of users) {
    if (withTokens.has(u.id)) continue;
    cost += (u.scans_fact_check || 0) * FALLBACK.fact_check + (u.scans_image || 0) * FALLBACK.image
          + (u.scans_text || 0) * FALLBACK.text + (u.follow_up_taps || 0) * FALLBACK.follow_up;
  }
  const revenue = purchases.reduce((a, x) => a + x.amountUSD, 0);

  // ── Scan feed ──────────────────────────────────────────────────────────
  // scan_started carries what went IN (voice? image? how long?), scan_completed
  // carries what came OUT (verdict, score, sources). They are separate events,
  // so pair each completion with the nearest preceding start by the same user.
  const starts = events
    .filter((e) => e.name === "scan_started")
    .map((e) => ({ uid: e.uid, at: ms(e.ts), p: e.params || {} }))
    .sort((a, b) => a.at - b.at);

  const startFor = (uid, at, durationMs) => {
    let best = null;
    for (const st of starts) {
      if (st.uid !== uid || st.at > at) continue;
      const gap = at - st.at;
      // A start belongs to this completion if it sits within the scan's own
      // duration plus a little slack — not just "most recent by this user".
      if (gap <= (durationMs || 0) + 20000 && (!best || st.at > best.at)) best = st;
    }
    return best;
  };

  // Queries live on the user doc, not the event, so they can only be matched by
  // user + kind, newest-first. Good enough to show what a person was checking.
  const queuePerUser = {};
  for (const u of users) {
    queuePerUser[u.id] = {
      fact_check: [...(u.fact_check_queries ?? [])].reverse(),
      text: [...(u.text_check_queries ?? [])].reverse(),
    };
  }

  const MODE_LABEL = {
    fact_check: "Fact check",
    image: "AI image detection",
    text: "AI text detection",
  };

  const scanFeed = events
    .filter((e) => e.name === "scan_completed" || e.name === "scan_failed")
    .map((e) => ({ e, at: ms(e.ts) }))
    .sort((a, b) => b.at - a.at)
    .slice(0, 80)
    .map(({ e, at }) => {
      const q = e.params || {};
      const st = startFor(e.uid, at, q.duration_ms);
      const mode = q.mode || st?.p.mode || "?";
      const queue = queuePerUser[e.uid];
      // Pop the next unused query of that kind for this user.
      let text = null;
      if (queue && (mode === "fact_check" || mode === "text") && queue[mode]?.length) {
        text = queue[mode].shift();
      }
      return {
        uid: e.uid,
        at,
        failed: e.name === "scan_failed",
        mode,
        modeLabel: MODE_LABEL[mode] || mode,
        verdict: q.verdict || (e.name === "scan_failed" ? "failed" : "—"),
        score: q.score,
        sources: q.sources_count,
        durationMs: q.duration_ms,
        reason: q.reason,
        // What went in, from the paired scan_started.
        input: st
          ? {
              hasImage: !!st.p.has_image,
              usedVoice: !!st.p.used_voice,
              chars: st.p.chars ?? null,
              isPro: !!st.p.is_pro,
              scansUsed: st.p.scans_used ?? null,
            }
          : null,
        text,
      };
    });

  // Every query people actually typed — the product's real demand signal.
  const queries = users.flatMap((u) => [
    ...(u.fact_check_queries ?? []).map((q) => ({ q, kind: "fact check", uid: u.id })),
    ...(u.text_check_queries ?? []).map((q) => ({ q, kind: "text check", uid: u.id })),
  ]);

  return {
    users,
    sessions,
    events,
    queries,
    purchases,
    acquisition: acquisitionOf(users),
    new7d: newSince(users, 7),
    scanFeed,
    money: { revenue, cost, pnl: revenue - cost },
    kpis: {
      users: users.length,
      sessions: sessions.length,
      events: events.length,
      scans: totalScans,
      scansPerUser: users.length ? totalScans / users.length : 0,
      sawPaywall: sawPaywall.length,
      converted: converted.length,
      convRate: sawPaywall.length ? converted.length / sawPaywall.length : 0,
      ranOut: users.filter((u) => u.ran_out_of_scans).length,
      medianSessionS: durations.length ? durations[Math.floor(durations.length / 2)] : 0,
    },
    scanMix: [
      { name: "fact check", value: users.reduce((a, u) => a + (u.scans_fact_check ?? 0), 0) },
      { name: "image", value: users.reduce((a, u) => a + (u.scans_image ?? 0), 0) },
      { name: "text", value: users.reduce((a, u) => a + (u.scans_text ?? 0), 0) },
    ].filter((s) => s.value > 0),
    byEvent: countBy(events, "name"),
    byCountry: countBy(sessions, "country", 8),
    byDevice: countBy(sessions, "device_model", 8),
    byVersion: countBy(sessions, "app_version", 5),
    perDay: daily(sessions, "started_at"),
    perUser: [...users]
      .map((u) => ({
        id: u.id,
        scans: scans(u),
        image: u.scans_image ?? 0,
        text: u.scans_text ?? 0,
        fact: u.scans_fact_check ?? 0,
        paywallViews: u.paywall_views ?? 0,
        converted: !!u.paywall_converted,
        ranOut: !!u.ran_out_of_scans,
        device: u.device || "—",
        os: u.os || "—",
        created: u.created_at,
        last: u.last_active,
      }))
      .sort((a, b) => b.scans - a.scans),
  };
}

// ── FAIKE drill-down ────────────────────────────────────────────────────────
// Sessions carry a `journey` array of step strings like
// "paywall_shown[first_launch]" — that IS the ordered trajectory, so the
// session page reads it directly rather than re-deriving one from events.

const faikeScans = (u) =>
  (u.scans_image ?? 0) + (u.scans_text ?? 0) + (u.scans_fact_check ?? 0);

/** Split "paywall_shown[first_launch]" into { step, detail }. */
export const parseStep = (s) => {
  const m = String(s).match(/^([^[]+)(?:\[(.*)\])?$/);
  return { step: m ? m[1] : String(s), detail: m && m[2] ? m[2] : null };
};

export async function faikeUser(uid) {
  const d = db("faike");
  const [userDoc, sessSnap, evSnap] = await Promise.all([
    d.collection("users").doc(uid).get(),
    d.collection("sessions").where("uid", "==", uid).limit(500).get(),
    d.collection("events").where("uid", "==", uid).limit(2000).get(),
  ]);
  if (!userDoc.exists) return null;

  const user = { id: userDoc.id, ...userDoc.data() };
  const sessions = rows(sessSnap).sort((a, b) => ms(b.started_at) - ms(a.started_at));
  const events = rows(evSnap).sort((a, b) => ms(b.ts) - ms(a.ts));

  const purchase = events.find((e) => e.name === "purchase_success");
  const revenue = purchase
    ? toUSD(purchase.params?.price ?? 0, purchase.params?.currency ?? "USD")
    : user.paywall_converted
      ? ({ yearly: 19.99, monthly: 3.99 }[user.conversion_plan || user.sub_plan || "monthly"] ?? 3.99)
      : 0;

  const FALLBACK = { fact_check: 0.04, image: 0.012, text: 0.005, follow_up: 0.002 };
  let cost = 0;
  const apiEvents = events.filter((e) => e.name === "api_usage");
  if (apiEvents.length) {
    for (const e of apiEvents) {
      const q = e.params || {};
      if (q.endpoint === "whisper") { cost += 0.006; continue; }
      const pr = q.model === "gpt-4o-mini" ? { in: 0.15, out: 0.6 } : { in: 2.5, out: 10 };
      cost += ((q.tokens_in || 0) / 1e6) * pr.in + ((q.tokens_out || 0) / 1e6) * pr.out;
      if (q.web_search) cost += 0.025;
    }
  } else {
    cost = (user.scans_fact_check || 0) * FALLBACK.fact_check
         + (user.scans_image || 0) * FALLBACK.image
         + (user.scans_text || 0) * FALLBACK.text
         + (user.follow_up_taps || 0) * FALLBACK.follow_up;
  }

  return {
    user,
    sessions,
    events,
    scans: faikeScans(user),
    money: { revenue, cost, pnl: revenue - cost },
    queries: [
      ...(user.fact_check_queries ?? []).map((q) => ({ q, kind: "fact check" })),
      ...(user.text_check_queries ?? []).map((q) => ({ q, kind: "text check" })),
    ],
  };
}

export async function faikeSession(id) {
  const d = db("faike");
  const doc = await d.collection("sessions").doc(id).get();
  if (!doc.exists) return null;
  const s = { id: doc.id, ...doc.data() };

  // Events the same user fired inside this session's window — gives each
  // journey step its real timestamp and params.
  const from = ms(s.started_at);
  const to = ms(s.last_event_at || s.ended_at) || from + 6 * 3600e3;
  const evSnap = await d.collection("events").where("uid", "==", s.uid).limit(2000).get();
  const events = rows(evSnap)
    .filter((e) => { const t = ms(e.ts); return t >= from - 2000 && t <= to + 2000; })
    .sort((a, b) => ms(a.ts) - ms(b.ts));

  return { session: s, events };
}

// ── Acquisition ─────────────────────────────────────────────────────────────
// "When do I gain users" — the one series every product should have. Products
// disagree about the signup field name, so try each in turn.

const CREATED_FIELDS = [
  "created_at", "firstSeenAt", "createdAt", "dateCreated",
  "firstSeenDay", "onboardedAt", "signupAt", "dateJoined",
];

const signupDay = (u) => {
  for (const f of CREATED_FIELDS) {
    const v = u[f];
    if (!v) continue;
    const t = ms(v);
    if (t) return new Date(t).toLocaleDateString("en-CA");
  }
  return null;
};

/** [{ day, newUsers, total }] — the shape AcquisitionChart wants. */
export function acquisitionOf(list, days = 45) {
  const byDay = new Map();
  for (const u of list) {
    const d = signupDay(u);
    if (d) byDay.set(d, (byDay.get(d) || 0) + 1);
  }
  const sorted = [...byDay.entries()].sort((a, b) => a[0].localeCompare(b[0]));
  let cum = 0;
  return sorted.map(([day, n]) => { cum += n; return { day, newUsers: n, total: cum }; }).slice(-days);
}

export const newSince = (list, n) =>
  list.filter((u) => {
    const d = signupDay(u);
    return d && Date.now() - Date.parse(d) < n * 864e5;
  }).length;

// ── FAIKE scan history ──────────────────────────────────────────────────────
// The full record of each scan, written by the app (ScanArchive) and by the
// one-time backfill of what each install already had stored locally.
// Everything needed to rebuild the result screen lives in one document.

const MODE_LABELS = {
  fact_check: "Fact check",
  image: "AI image detection",
  text: "AI text detection",
  link: "Link check",
};

/** One Firestore doc -> the shape every scan view uses. */
function mapScan(d) {
  return {
    id: d.id,
    itemId: d.item_id,
    uid: d.uid,
    at: ms(d.at),
    mode: d.mode || "?",
    modeLabel: MODE_LABELS[d.mode] || d.mode || "?",
    preview: d.preview || "",
    text: d.text || "",
    question: d.question || "",
    score: d.score,
    verdict: d.verdict || "—",
    reason: d.reason || "",
    detail: d.detail || "",
    sources: d.sources || [],
    sourcesCount: d.sources_count ?? (d.sources || []).length,
    criteria: d.criteria || [],
    followUps: (d.follow_ups || [])
      .map((f) => ({ ...f, at: ms(f.at) }))
      .sort((a, b) => a.at - b.at),
    imagePath: d.image_path || null,
    hadImage: !!d.had_image,
    usedVoice: !!d.used_voice,
    durationMs: d.duration_ms ?? null,
    passes: d.passes ?? null,
    divergence: d.divergence ?? null,
    isPro: !!d.is_pro,
    backfilled: !!d.backfilled,
  };
}

export async function faikeScanHistory({ limit = 200 } = {}) {
  try {
    const snap = await db("faike")
      .collection("scan_history")
      .orderBy("at", "desc")
      .limit(limit)
      .get();
    return { scans: snap.docs.map((d) => mapScan({ id: d.id, ...d.data() })), ready: true };
  } catch {
    // Collection does not exist yet, or its index is still building.
    return { scans: [], ready: false };
  }
}

export async function faikeScan(docId) {
  const doc = await db("faike").collection("scan_history").doc(docId).get();
  if (!doc.exists) return null;
  return mapScan({ id: doc.id, ...doc.data() });
}

// ── Sessions <-> scans ──────────────────────────────────────────────────────
// Scans do not carry a session id yet, so pair them by user and time: a scan
// belongs to the session whose window it falls inside. Works for backfilled
// records too, which never had a session to belong to in the first place.

export function attachScans(sessions, scans) {
  return sessions.map((s) => {
    const from = ms(s.started_at);
    const to = ms(s.last_event_at || s.ended_at) || from + 6 * 3600e3;
    const mine = scans
      .filter((x) => x.uid === s.uid && x.at >= from - 2000 && x.at <= to + 2000)
      .sort((a, b) => a.at - b.at);
    return { ...s, scans: mine };
  });
}

/* ── Surrender — the nightly confession app ──────────────────────────── */
//
// The only product with no API cost: the seven-day plan is generated on device
// from a fixed library, so there is no model to pay for. Revenue is margin.
export async function surrender() {
  const d = db("surrender");
  const [users, events, sessions] = await Promise.all([
    d.collection("users").get().then(rows),
    d.collection("events").orderBy("ts", "desc").limit(3000).get().then(rows),
    d.collection("sessions").orderBy("started_at", "desc").limit(500).get().then(rows),
  ]);

  const named = (n) => events.filter((e) => e.name === n);
  const p = (e) => e.params || {};

  // ── The onboarding funnel ────────────────────────────────────────────
  // Each step is counted by DISTINCT uid, not by event, so a back-and-forth
  // through the flow does not inflate a step above the one before it.
  const STEPS = [
    ["welcome", "Welcome"], ["name", "Name"], ["proof", "Proof"],
    ["tradition", "Tradition"], ["closeness", "Closeness"], ["focus", "Pick three"],
    ["reframe", "Reframe"], ["confession", "Confession"], ["building", "Building"],
    ["commitment", "Commitment"], ["plan_reveal", "Plan reveal"], ["app_lock", "App lock"],
    ["reminders", "Reminders"], ["paywall", "Paywall"], ["gift", "Gift"],
  ];
  const reachedBy = new Map(STEPS.map(([k]) => [k, new Set()]));
  for (const e of named("onboarding_step")) {
    const k = p(e).step_name;
    if (reachedBy.has(k) && e.uid) reachedBy.get(k).add(e.uid);
  }
  const top = reachedBy.get("welcome")?.size || users.length || 1;
  const funnel = STEPS.map(([k, label]) => {
    const n = reachedBy.get(k).size;
    return { key: k, name: label, value: n, pct: Math.round((n / top) * 1000) / 10 };
  });
  // Biggest single drop, which is the only number worth acting on today.
  let worst = { name: "—", lost: 0, pct: 0 };
  for (let i = 1; i < funnel.length; i++) {
    const lost = funnel[i - 1].value - funnel[i].value;
    if (lost > worst.lost) {
      worst = { name: funnel[i].name, from: funnel[i - 1].name, lost,
                pct: funnel[i - 1].value ? Math.round((lost / funnel[i - 1].value) * 1000) / 10 : 0 };
    }
  }

  // Median seconds per step — a step people stall on is a step they are reading
  // or a step confusing them, and the funnel alone cannot tell you which.
  const dwellByStep = new Map();
  for (const e of named("onboarding_step_left")) {
    const k = p(e).step_name, s = p(e).seconds;
    if (!k || typeof s !== "number") continue;
    if (!dwellByStep.has(k)) dwellByStep.set(k, []);
    dwellByStep.get(k).push(s);
  }
  const median = (a) => (a.length ? a.slice().sort((x, y) => x - y)[Math.floor(a.length / 2)] : 0);
  const dwell = STEPS
    .map(([k, label]) => ({ name: label, value: median(dwellByStep.get(k) || []) }))
    .filter((r) => r.value > 0);

  // ── What they answered ───────────────────────────────────────────────
  const answers = {
    tradition: countBy(users, "tradition"),
    closeness: countBy(users, "closeness"),
    theme: countBy(users, "theme"),
    focus: countBy(
      users.flatMap((u) => String(u.focuses || "").split(",").filter(Boolean).map((f) => ({ f }))),
      "f",
    ),
    arrow: countBy(named("check_in_saved").map((e) => ({ a: p(e).arrow })), "a"),
    hardNight: countBy(named("hard_nights_situation").map((e) => ({ s: p(e).situation })), "s"),
    country: countBy(users, "country"),
  };

  // ── The confession, which is the product's one irreversible step ─────
  const wrote = users.filter((u) => u.confession_written === true);
  const skipped = named("confession_skipped").length;
  const lengths = wrote.map((u) => u.confession_len || 0).filter(Boolean);
  const confession = {
    written: wrote.length,
    skipped,
    rate: wrote.length + skipped ? Math.round((wrote.length / (wrote.length + skipped)) * 1000) / 10 : 0,
    medianLength: median(lengths),
  };

  // ── Paywall behaviour ────────────────────────────────────────────────
  const views = named("paywall_view");
  const closes = named("paywall_closed");
  const dwellSecs = closes.map((e) => p(e).seconds).filter((s) => typeof s === "number");
  const paywall = {
    views: views.length,
    viewers: new Set(views.map((e) => e.uid)).size,
    dismissals: closes.length,
    medianSecondsBeforeDismiss: median(dwellSecs),
    underFiveSeconds: dwellSecs.filter((s) => s < 5).length,
    byMode: countBy(views.map((e) => ({ m: p(e).mode })), "m"),
    planTaps: countBy(named("paywall_plan_selected").map((e) => ({ x: p(e).product_id })), "x"),
    trialToggledOff: named("paywall_trial_toggled").filter((e) => p(e).on === false).length,
    giftsClaimed: named("gift_claimed").length,
  };

  // ── Money. No API cost to subtract: the generator runs on device. ─────
  const purchases = named("purchase_succeeded").map((e) => {
    const q = p(e);
    const cur = q.currency || "USD";
    const native = typeof q.value === "number" ? q.value : 0;
    return { uid: e.uid, at: ms(e.ts), plan: q.product_id || "unknown", native, currency: cur,
             amountUSD: toUSD(native, cur) };
  });
  const revenue = purchases.reduce((a, x) => a + x.amountUSD, 0);
  const apiCost = 0;

  // Marketing spend is not something the app can know. It is read from a
  // `spend` collection you write to (one row per day per channel); with none,
  // CAC and ROAS stay null rather than guessing.
  let spendRows = [];
  try { spendRows = await d.collection("spend").get().then(rows); } catch { spendRows = []; }
  const spend = spendRows.reduce((a, r) => a + (Number(r.amount_usd) || 0), 0);
  // Distinct anonymous uids, not installs: Firebase Auth keeps its user in the
  // keychain, which survives an app uninstall, so a reinstall is the same person.
  // That is the more useful number — it just is not "downloads".
  const installs = users.length;
  const money = {
    revenue, apiCost, spend,
    profit: revenue - apiCost - spend,
    installs,
    rpd: installs ? revenue / installs : 0,
    cac: spend && installs ? spend / installs : null,
    roas: spend ? revenue / spend : null,
    conversions: purchases.length,
    conversionRate: installs ? Math.round((purchases.length / installs) * 1000) / 10 : 0,
    byPlan: countBy(purchases, "plan"),
  };

  // ── Retention: the nightly loop is the whole product ─────────────────
  const checkIns = named("check_in_saved");
  const retention = {
    checkIns: checkIns.length,
    withNote: checkIns.filter((e) => p(e).has_note === true).length,
    daysCompleted: named("day_completed").length,
    prayersCompleted: named("prayer_completed").length,
    prayersAbandoned: named("prayer_abandoned").length,
    medianPrayerSeconds: median(named("prayer_completed").map((e) => p(e).seconds).filter(Boolean)),
    hardNightsOpens: named("hard_nights_opened").length,
    milestones: countBy(named("streak_milestone").map((e) => ({ n: p(e).nights })), "n"),
    repairsUsed: named("repair_used").length,
    repairsBought: named("repair_purchased").length,
    pushOpens: countBy(named("notification_opened").map((e) => ({ k: p(e).kind })), "k"),
    weeksStarted: named("new_week_started").length,
    streakDistribution: countBy(users, (u) => {
      const n = u.streak ?? 0;
      return n === 0 ? "0" : n <= 2 ? "1-2" : n <= 6 ? "3-6" : n <= 13 ? "7-13" : n <= 29 ? "14-29" : "30+";
    }),
  };

  return {
    users, events, sessions,
    funnel, worst, dwell, answers, confession, paywall, money, retention,
    installsDaily: daily(users, "created_at"),
    revenueDaily: daily(purchases.map((x) => ({ at: x.at })), "at"),
    purchases,
  };
}

/* ── Complainr ───────────────────────────────────────────────────────── */

/**
 * The app writes `users` (profile plus a rolling `usage` summary),
 * `users/{uid}/complaints`, and a root `events` stream. Categories are
 * per-user rather than a fixed taxonomy, so the category chart is an
 * aggregate of vocabularies people invented, not of ours.
 */
export async function complainr() {
  const d = db("complainr");
  const [users, events] = await Promise.all([
    d.collection("users").get().then(rows),
    d.collection("events").orderBy("ts", "desc").limit(3000).get().then(rows),
  ]);

  const named = (n) => events.filter((e) => e.name === n);
  const p = (e) => e.params || {};
  const uniq = (list) => new Set(list.map((e) => e.uid).filter(Boolean)).size;
  const pct = (n, of) => (of > 0 ? Math.round((n / of) * 100) : 0);

  // ── Registration, the core loop ──────────────────────────────────────
  const registered = named("complaint_registered");
  const repeated = named("complaint_repeated");

  const bySource = {};
  const byCategory = {};
  let withContext = 0;
  let withTears = 0;
  let withImage = 0;
  for (const e of registered) {
    const q = p(e);
    bySource[q.source || "typed"] = (bySource[q.source || "typed"] || 0) + 1;
    if (q.category) byCategory[q.category] = (byCategory[q.category] || 0) + 1;
    if (q.has_context) withContext++;
    if (q.tears) withTears++;
    if (q.has_image) withImage++;
  }
  const chart = (obj, limit = 10) =>
    Object.entries(obj)
      .sort((a, b) => b[1] - a[1])
      .slice(0, limit)
      .map(([name, value]) => ({ name, value }));

  // ── The paywall ──────────────────────────────────────────────────────
  const shown = named("paywall_shown");
  const bought = named("purchase_completed");
  const paywallBySource = {};
  for (const e of shown) {
    const k = p(e).source || "unknown";
    paywallBySource[k] = (paywallBySource[k] || 0) + 1;
  }

  // ── The growth loop ──────────────────────────────────────────────────
  const reportsOpened = named("report_opened");
  const reportsSent = named("report_shared");

  // ── Usage summaries the app maintains per user ───────────────────────
  const usage = users.map((u) => u.usage || {}).filter((u) => u.totalComplaints != null);
  const sum = (k) => usage.reduce((t, u) => t + (u[k] || 0), 0);
  const totalComplaints = sum("totalComplaints");
  const avgPerUser = usage.length ? (totalComplaints / usage.length).toFixed(1) : "0";
  const perDay = usage.length
    ? (usage.reduce((t, u) => t + (u.averagePerDay || 0), 0) / usage.length).toFixed(2)
    : "0";

  // Week over week across the whole base — the number the product promises
  // to move. Negative is the good direction here.
  const thisWeek = sum("thisWeek");
  const lastWeek = sum("lastWeek");

  return {
    counts: {
      users: users.length,
      registered: registered.length,
      repeated: repeated.length,
      totalComplaints,
      avgPerUser,
      perDay,
      thisWeek,
      lastWeek,
      weekDelta: thisWeek - lastWeek,
      tearsRate: pct(withTears, registered.length),
      contextRate: pct(withContext, registered.length),
      imageRate: pct(withImage, registered.length),
      distinct: sum("distinctComplaints"),
    },
    sources: chart(bySource),
    categories: chart(byCategory, 12),
    paywall: {
      shown: shown.length,
      viewers: uniq(shown),
      bought: bought.length,
      conversion: pct(uniq(bought), uniq(shown)),
      bySource: chart(paywallBySource),
    },
    reports: {
      opened: reportsOpened.length,
      sent: reportsSent.length,
      sendRate: pct(reportsSent.length, reportsOpened.length),
      senders: uniq(reportsSent),
    },
    ai: {
      advice: named("advice_requested").length,
      adviceOnDevice: named("advice_requested").filter((e) => p(e).path === "local").length,
      voice: named("voice_transcribed").length,
      enriched: named("enrichment_completed").length,
      newCategories: named("enrichment_completed").filter((e) => p(e).new_category).length,
    },
    installsDaily: daily(users, "lastSeen", 30),
    registrationsDaily: daily(registered, "ts", 30),
  };
}

/* ── Hineni ──────────────────────────────────────────────────────────── */

export async function hineni() {
  const d = db("hineni");
  const [users, events, sessions] = await Promise.all([
    d.collection("users").get().then(rows),
    d.collection("events").orderBy("ts", "desc").limit(3000).get().then(rows),
    d.collection("sessions").orderBy("started_at", "desc").limit(500).get().then(rows),
  ]);

  const named = (n) => events.filter((e) => e.name === n);
  const p = (e) => e.params || {};
  const median = (a) => (a.length ? a.slice().sort((x, y) => x - y)[Math.floor(a.length / 2)] : 0);

  // ── The onboarding funnel ────────────────────────────────────────────
  // Counted by DISTINCT uid, not by event, so going back and forth through
  // the flow cannot push a step above the one before it.
  const STEPS = [
    ["welcome", "Welcome"], ["name", "Name"], ["proof", "Proof"],
    ["movement", "Movement"], ["standing", "Standing"], ["practices", "Pick three"],
    ["reframe", "Reframe"], ["opening_line", "One sentence"], ["place", "Place"],
    ["building", "Building"], ["path_reveal", "Path reveal"], ["reminders", "Reminders"],
    ["app_lock", "App lock"], ["paywall", "Paywall"], ["gift", "Gift"],
  ];
  const reachedBy = new Map(STEPS.map(([k]) => [k, new Set()]));
  for (const e of named("onboarding_step")) {
    const k = p(e).step_name;
    if (reachedBy.has(k) && e.uid) reachedBy.get(k).add(e.uid);
  }
  const top = reachedBy.get("welcome")?.size || users.length || 1;
  const funnel = STEPS.map(([k, label]) => {
    const n = reachedBy.get(k).size;
    return { key: k, name: label, value: n, pct: Math.round((n / top) * 1000) / 10 };
  });
  let worst = { name: "—", lost: 0, pct: 0 };
  for (let i = 1; i < funnel.length; i++) {
    const lost = funnel[i - 1].value - funnel[i].value;
    if (lost > worst.lost) {
      worst = { name: funnel[i].name, from: funnel[i - 1].name, lost,
                pct: funnel[i - 1].value ? Math.round((lost / funnel[i - 1].value) * 1000) / 10 : 0 };
    }
  }

  const dwellByStep = new Map();
  for (const e of named("onboarding_step_left")) {
    const k = p(e).step_name, s = p(e).seconds;
    if (!k || typeof s !== "number") continue;
    if (!dwellByStep.has(k)) dwellByStep.set(k, []);
    dwellByStep.get(k).push(s);
  }
  const dwell = STEPS
    .map(([k, label]) => ({ name: label, value: median(dwellByStep.get(k) || []) }))
    .filter((r) => r.value > 0);

  // ── What they told us ────────────────────────────────────────────────
  const answers = {
    movement: countBy(users, "movement"),
    standing: countBy(users, "standing"),
    theme: countBy(users, "theme"),
    practice: countBy(
      users.flatMap((u) => String(u.practices || "").split(",").filter(Boolean).map((f) => ({ f }))),
      "f",
    ),
    kavanah: countBy(named("check_in_saved").map((e) => ({ a: p(e).kavanah })), "a"),
    script: countBy(users, "prop_script"),
    schedule: countBy(users, "prop_schedule"),
    country: countBy(users, "country"),
    place: countBy(users, "prop_place"),
  };

  // ── The free utility, which is the whole acquisition argument ────────
  // Candle lighting and the zmanim are given away deliberately. If they are
  // not what people open the app for, the strategy is wrong and this is
  // where that shows up first.
  const utility = {
    candleViews: named("candle_time_viewed").length,
    candleViewers: new Set(named("candle_time_viewed").map((e) => e.uid)).size,
    zmanimViews: named("zmanim_viewed").length,
    zmanimViewers: new Set(named("zmanim_viewed").map((e) => e.uid)).size,
    brachaSearches: named("bracha_searched").length,
    // Searches that found nothing are the food list's to-do list. The query
    // itself is never recorded, only its length and whether it hit.
    brachaMisses: named("bracha_searched").filter((e) => p(e).results === 0).length,
    brachaByCategory: countBy(named("bracha_viewed").map((e) => ({ c: p(e).category })), "c"),
    prayerOpens: countBy(named("prayer_opened").map((e) => ({ k: p(e).prayer })), "k"),
    readAloud: named("prayer_read_aloud").length,
    readAloudHebrew: named("prayer_read_aloud").filter((e) => p(e).hebrew === true).length,
    shabbatModeShows: named("shabbat_mode_shown").length,
    omerCounts: named("omer_counted").length,
  };

  // ── Location: how many actually grant it ─────────────────────────────
  const granted = named("location_granted");
  const location = {
    asked: granted.length,
    granted: granted.filter((e) => p(e).granted === true).length,
    byMethod: countBy(named("place_chosen").map((e) => ({ m: p(e).method })), "m"),
  };

  // ── Tefillin ─────────────────────────────────────────────────────────
  const tefillin = {
    opens: named("tefillin_opened").length,
    completions: named("tefillin_completed").length,
    completers: new Set(named("tefillin_completed").map((e) => e.uid)).size,
    // How far people get before abandoning the wrap. Step 4 is the seven
    // wraps and step 6 is the second blessing; a cliff at either is a
    // usability problem rather than a religious one.
    furthestStep: countBy(named("tefillin_step").map((e) => ({ s: p(e).step_index })), "s"),
    blocked: countBy(named("tefillin_blocked_today").map((e) => ({ r: p(e).reason })), "r"),
    streaks: countBy(users, (u) => {
      const n = u.tefillin_streak ?? 0;
      return n === 0 ? "0" : n <= 2 ? "1-2" : n <= 6 ? "3-6" : n <= 13 ? "7-13" : "14+";
    }),
  };

  // ── Holiday guides ───────────────────────────────────────────────────
  // Which intensity people pick is the most interesting question in the app:
  // the whole design bet is that offering a ten-minute version gets more
  // people doing something than describing the full observance would.
  const guideOpens = named("holiday_guide_opened");
  const holidays = {
    guideOpens: guideOpens.length,
    guideViewers: new Set(guideOpens.map((e) => e.uid)).size,
    byHoliday: countBy(guideOpens.map((e) => ({ h: p(e).holiday })), "h"),
    byLevel: countBy(named("holiday_level_chosen").map((e) => ({ l: p(e).level })), "l"),
    // How far ahead people look. A spike at 0 means they only find out on the
    // day, which is what the countdown card exists to fix.
    daysAhead: countBy(
      guideOpens.map((e) => {
        const n = p(e).days_away;
        if (typeof n !== "number" || n < 0) return { d: "unknown" };
        return { d: n === 0 ? "on the day" : n === 1 ? "day before" : n <= 7 ? "within a week" : "over a week" };
      }),
      "d",
    ),
    viewed: countBy(named("holiday_viewed").map((e) => ({ h: p(e).holiday })), "h"),
  };

  // ── Paywall ──────────────────────────────────────────────────────────
  const views = named("paywall_view");
  const closes = named("paywall_closed");
  const dwellSecs = closes.map((e) => p(e).seconds).filter((s) => typeof s === "number");
  const paywall = {
    views: views.length,
    viewers: new Set(views.map((e) => e.uid)).size,
    dismissals: closes.length,
    medianSecondsBeforeDismiss: median(dwellSecs),
    underFiveSeconds: dwellSecs.filter((s) => s < 5).length,
    byMode: countBy(views.map((e) => ({ m: p(e).mode })), "m"),
    planTaps: countBy(named("paywall_plan_selected").map((e) => ({ x: p(e).product_id })), "x"),
    trialToggledOff: named("paywall_trial_toggled").filter((e) => p(e).on === false).length,
    giftsClaimed: named("gift_claimed").length,
  };

  // ── Money. No API cost: the path generator runs on device. ───────────
  const purchases = named("purchase_succeeded").map((e) => {
    const q = p(e);
    const cur = q.currency || "USD";
    const native = typeof q.value === "number" ? q.value : 0;
    return { uid: e.uid, at: ms(e.ts), plan: q.product_id || "unknown", native, currency: cur,
             amountUSD: toUSD(native, cur) };
  });
  const revenue = purchases.reduce((a, x) => a + x.amountUSD, 0);
  const apiCost = 0;

  let spendRows = [];
  try { spendRows = await d.collection("spend").get().then(rows); } catch { spendRows = []; }
  const spend = spendRows.reduce((a, r) => a + (Number(r.amount_usd) || 0), 0);
  const installs = users.length;
  const money = {
    revenue, apiCost, spend,
    profit: revenue - apiCost - spend,
    installs,
    rpd: installs ? revenue / installs : 0,
    cac: spend && installs ? spend / installs : null,
    roas: spend ? revenue / spend : null,
    conversions: purchases.length,
    conversionRate: installs ? Math.round((purchases.length / installs) * 1000) / 10 : 0,
    byPlan: countBy(purchases, "plan"),
  };

  // ── Retention: the daily answer is the loop ──────────────────────────
  const checkIns = named("check_in_saved");
  const retention = {
    checkIns: checkIns.length,
    withNote: checkIns.filter((e) => p(e).has_note === true).length,
    daysCompleted: named("day_completed").length,
    pathsStarted: named("new_path_started").length,
    lockedDayTaps: named("day_locked_tapped").length,
    milestones: countBy(named("streak_milestone").map((e) => ({ n: p(e).days })), "n"),
    restoresUsed: named("restore_used").length,
    restoresBought: named("restore_purchased").length,
    pushOpens: countBy(named("notification_opened").map((e) => ({ k: p(e).kind })), "k"),
    yahrzeitsAdded: named("yahrzeit_added").length,
    withYahrzeit: users.filter((u) => u.has_yahrzeit === true).length,
    streakDistribution: countBy(users, (u) => {
      const n = u.streak ?? 0;
      return n === 0 ? "0" : n <= 2 ? "1-2" : n <= 6 ? "3-6" : n <= 13 ? "7-13" : n <= 29 ? "14-29" : "30+";
    }),
  };

  return {
    users, events, sessions,
    funnel, worst, dwell, answers, utility, location, tefillin, holidays,
    paywall, money, retention,
    installsDaily: daily(users, "created_at"),
    revenueDaily: daily(purchases.map((x) => ({ at: x.at })), "at"),
    purchases,
  };
}
