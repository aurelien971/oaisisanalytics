...
export async function surrender() {
  ...
  // ── ATT, because the whole paid-acquisition plan depends on it ────────
  const att = countBy(named("att_result").map((e) => ({ s: p(e).status })), "s");

  return {
    users, events, sessions,
    funnel, worst, dwell, answers, confession, paywall, money, retention, att,
